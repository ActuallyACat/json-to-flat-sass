...
export const jsonToFlatSass = async ({
  ...
  flattenedInput = flattenInputToString(parsedString, caseType, variablePrefix);
...
const formatSubstring = (name, caseType, key) => {
  const substring = `${name}-${key}`;
  if (caseType === 'kebab') { 
    return kebabCase(substring);
  }
  if (caseType === 'snake') { 
    return snakeCase(substring);
  }
  if (caseType === 'camel') { 
...
export const flattenInputToString = (
  ...
  const dive = (input: {[index: string]: any}, name: string): void => {
    ...
    Object.keys(input).forEach(key => {
      let concatName = name === '' ? key : formatSubstring(name, caseType, key);
